...
class FEARTracker(Tracker):
    ...
    def set_state(self, state, feat):
            
            self.tracking_state.bbox=state.bbox
            self.tracking_state.mapping=state.mapping
            self.tracking_state.mean_color=state.mean_color
            self.tracking_state.prev_size=state.prev_size
            self.tracking_state.paths=state.paths

            self._template_features=feat
    ...
    def update(self, image: np.ndarray, *kw) -> Dict[str, Any]:
        ...
        return dict(bbox=pred_bbox), score
